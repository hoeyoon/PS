...
int* solution(const char* keyinput[], size_t keyinput_len, int board[], size_t board_len) {
    ...
    for(int i = 0; i < keyinput_len; i++){
        if(strcmp(keyinput[i], "left") == 0 && answer[0] > -x){
            answer[0]--;
        }
        else if(strcmp(keyinput[i], "right") == 0 && answer[0] < x){
            answer[0]++;
        }
        else if(strcmp(keyinput[i], "up") == 0 && answer[1] < y){
            answer[1]++;
        }
        else if(strcmp(keyinput[i], "down") == 0 && answer[1] > -y){
            answer[1]--;
        }
    }
    return answer;
